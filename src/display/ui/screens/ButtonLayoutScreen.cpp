...
void ButtonLayoutScreen::generateHeader() {
	...
	switch (Gamepad::resolveSOCDMode(gamepad->getOptions()))
	{
		...
		case SOCD_MODE_BYPASS:                statusBar += " xxxxxxxxSOCD-X"; break;
	}
	if (macroEnabled)
		statusBar += " M";
#endif
    uint8_t CSmode = Storage::getInstance().getCSmode();
    switch (CSmode)
	{
		case CSMODE_GLOC:           statusBar += "          G-LOC"; break;
		case CSMODE_PANZER_DRAGOON: statusBar += " PANZER DRAGOON"; break;
		case CSMODE_SPACE_HARRIER:  statusBar += "  SPACE HARRIER"; break;
		case CSMODE_TEMPEST_4000:   statusBar += "   TEMPEST 4000"; break;
		case CSMODE_VIRTUA_RACING:  statusBar += "  VIRTUA RACING"; break;
        default:                    statusBar += "        DEFAULT"; break;
	}
}
